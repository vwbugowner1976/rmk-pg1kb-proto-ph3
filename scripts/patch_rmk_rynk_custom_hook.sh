...
if [ -n "${PG1KB_RMK_ROOT:-}" ]; then
  ...
  RMK_ROOT="$(find "$HOME/.cargo/registry/src" -maxdepth 2 -type d -name 'rmk-0.9.0' 2>/dev/null | sort | tail -n 1)"
fi

if [ -z "$RMK_ROOT" ] || [ ! -f "$RMK_ROOT/Cargo.toml" ]; then
  echo "RMK 0.9.0 root not found." >&2
  exit 1
fi
...
if marker not in text:
    ...
    for anchor, code in [(import_anchor, 2), (const_anchor, 3), (dispatch_anchor, 4)]:
...
'''
    ...
    print("Patched RMK Rynk custom-command hook")
else:
    print(f"Rynk custom hook already present: {rynk}")
...
if marker not in verify_rynk:
    print("VERIFY FAILED: RMK custom hook missing", file=sys.stderr)
    raise SystemExit(7)

print("VERIFY OK: active RMK custom Rynk hook + host re-export")
print(f"RMK root: {rynk.parents[2]}")
PY
